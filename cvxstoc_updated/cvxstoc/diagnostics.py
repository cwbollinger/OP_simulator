...
class ConstrDiagnostics:
    ...
    def est_optval(self, how, obj, constrs):
...
class UnconstrDiagnostics:
    ...
    def est_optgap(self, how, stoch_objf, num_replications, num_samples, alpha):
        (ignore, fhatmean, fhatvar, ignore, ignore) = self.est_cost(how, stoch_objf, num_replications, alpha)
        (pstarhatmean, pstarhatvar, ignore, ignore) = self.est_optval(how, stoch_objf, num_replications, num_samples, alpha)
    ...
    def est_optgap_core(self, fhatmean, fhatvar, pstarhatmean, pstarhatvar, alpha):
        ...
        pm = scipy.stats.norm.ppf( 1.0 - (1.0-alpha)/2.0 ) * np.sqrt( fhatvar + pstarhatvar )
    ...
    def est_cost(self, stoch_objf, num_replications, alpha):
        ...
        fhats = [cvx.stochastic.expectation(stoch_objf,1).value for i in range(num_replications)]
    ...
    def est_optval(self, how_est_optval, stoch_objf, num_replications, num_samples, how_conf_int, alpha, A=None, b=None):
        ...
        if how_est_optval == HowEstUnconstrOptVal.AVERAGE_SAAS:
            ...
            if how_conf_int == ConfInt.T:
                ...
                for i in range(num_replications):
                    prob = cvx.Problem( cvx.Minimize(cvx.stochastic.expectation(stoch_objf,num_samples)) )
                    prob.solve()
                ...
                for idx in range(num_samples):
                    objf += cvx.sum_squares(A_samples[idx,:,:]*x-b_samples[idx,:])
                prob = cvx.Problem(cvx.Minimize((1.0/num_samples)*objf))
                prob.solve()
                pstarhatmean = prob.value

                pstarhats = []
                for i in range(num_replications): # Each iteration here is a bootstrap replication
            ...
            prob = cvx.Problem( cvx.Minimize(cvx.stochastic.expectation(stoch_objf,want_mf=True)) )
            prob.solve()
            pstarhatmean = prob.value
    ...
    @staticmethod
    def get_critval(alpha, how=HowGetCritVal.NORMAL, df=None): # Note: alpha is (expected to be) small here
...
class TestDiagnostics:
    ...
    def test_least_squares_plot_saa_quality(self, A, b, P, q, r, num_samples_list, fn, num_replications, alpha):
        dims = A.shape
        (m,n) = dims.rows, dims.cols
        ...
        for num_samples in num_samples_list:
            print("Solving a SAA to the stochastic least squares problem with [{}] Monte Carlo samples.".format(num_samples))
            pstarhats = []
            for i in range(num_replications):
                x = cvx.Variable(n)
                prob = cvx.Problem( cvx.Minimize(cvx.stochastic.expectation(cvx.sum_squares(A*x-b),num_samples)) )
                prob.solve()
        ...
        ax.plot( num_samples_list, pstarhat_avgs, '-', label=r"$\hat{p}^*_N$", color="blue", lw=self.linewidth )
        ax.fill_between( num_samples_list, pstarhat_ucbs, pstarhat_lcbs, facecolor="lightskyblue", alpha=0.5 )
        ax.plot( num_samples_list, pstars, '-', label=r"$p^*$", color="crimson", lw=self.linewidth )

        ax.legend(loc="lower right", fontsize=self.fontsize)
        ax.set_xlabel(r"\# Monte Carlo samples $N$", fontsize=self.fontsize) # number of Monte Carlo samples $N$
    ...
    def test_least_squares_plot_optval_ests(self, A, b, P, q, r, num_replications_list, num_samples_list, alpha, fn1, fn2, fn3, fn4, fn5, fhatmeans, fstars):
        ...
        for i in range(X): #range(X-1,-1,-1):
            ...
            for j in range(Y): #range(Y-1,-1,-1):
                ...
                stoch_objf = cvx.sum_squares(A*x-b)
                print("Averaging the opt. vals. of [%d] SAAs with [%d] Monte Carlo samples to the stoch. least squares problem; requesting t-based conf. ints." % (num_replications, num_samples))
                (pstarhatmean, pstarhatvar_ignore, pstarhatucb, pstarhatlcb) = self.unconstr_diag.est_optval(HowEstUnconstrOptVal.AVERAGE_SAAS, stoch_objf, num_replications, num_samples, ConfInt.T, alpha)
                ...
                error = gaphat - gap
                print("Error between a gap estimator with [%d] reps. and [%d] samples and the true gap is [%f]." % (num_replications, num_samples, error))
        ...
        (pstarhat_mf, ignore, ignore, ignore) = self.unconstr_diag.est_optval(HowEstUnconstrOptVal.MEANFIELD, stoch_objf, None, None, None, None)
        ...
        (num_reps_list_meshed, num_samps_list_meshed) = np.meshgrid(num_replications_list, num_samples_list)
        surf = ax.plot_surface(num_reps_list_meshed, num_samps_list_meshed, errors, rstride=1, cstride=1, cmap=cm.autumn, linewidth=0, antialiased=False, alpha=0.75)
        ...
        stoch_objf = cvx.sum_squares(A*x-b)
        print("Averaging the opt. vals. of [%d] SAAs with [%d] Monte Carlo samples to the stoch. least squares problem; requesting t-based conf. ints." % (num_reps_fixed, num_samps_addl))
        (pstarhatmean, pstarhatvar_ignore, pstarhatucb, pstarhatlcb) = self.unconstr_diag.est_optval(HowEstUnconstrOptVal.AVERAGE_SAAS, stoch_objf, num_reps_fixed, num_samps_addl, ConfInt.T, alpha)

        gaphat_addl = fhatmeans[idx] - pstarhatmean
        print("Error between a gap estimator with [%d] reps. and [%d] samples and the true gap is [%f]." % (num_reps_fixed, num_samps_addl, gaphat_addl - gap))
        ...
        fig, ax = plt.subplots()
        ax.plot( num_samples_list, gaphats_plot, "-", label=r"$\hat{\Delta}_{M,N}$", color="blue", lw=self.linewidth )
        ax.plot( num_samples_list, gaps, "-", label=r"$\Delta$", color="crimson", lw=self.linewidth )

        ax.legend(loc="upper right", fontsize=self.fontsize)
        ax.set_xlabel(r"\# samples $N$ (\# replications $M$ = %d fixed)" % num_reps_fixed, fontsize=self.fontsize)
        ax.set_xscale("log")
        for label in (ax.get_xticklabels() + ax.get_yticklabels()):
            label.set_fontsize(self.fontsize_axis)
        ...
        fig, ax = plt.subplots()
        plt.rc("text", usetex=True)
        ax.plot( num_samples_list, pstarhats_plot, "-", label=r"$\hat{p}^*_{M,N}$", color="blue", lw=self.linewidth )
        ax.fill_between(num_samples_list, pstarhat_ucbs_plot, pstarhat_lcbs_plot, facecolor="lightskyblue", alpha=0.5)
        ax.plot(num_samples_list, pstarhats_mf, "-", label=r"$\hat{p}^*_{\mathrm{mf}}$", color="forestgreen", lw=self.linewidth)
        ax.plot(num_samples_list, pstars, "-", label=r"$p^*$", color="crimson", lw=self.linewidth )
        ...
        stoch_objf = cvx.sum_squares(A*x-b)
        print("Averaging the opt. vals. of [%d] SAAs with [%d] Monte Carlo samples to the stoch. least squares problem; requesting t-based conf. ints." % (num_reps_addl, num_samps_fixed))
        (pstarhatmean, pstarhatvar_ignore, pstarhatucb, pstarhatlcb) = self.unconstr_diag.est_optval(HowEstUnconstrOptVal.AVERAGE_SAAS, stoch_objf, num_reps_addl, num_samps_fixed, ConfInt.T, alpha)
        ...
        ax.plot( num_replications_list, pstarhats_plot, "-", label=r"$\hat{p}^*_{M,N}$", color="blue", lw=self.linewidth )
        ax.fill_between(num_replications_list, pstarhat_ucbs_plot, pstarhat_lcbs_plot, facecolor="lightskyblue", alpha=0.5)
        ...
        ax.plot(num_replications_list, pstarhats_mf, "-", label=r"$\hat{p}^*_{\mathrm{mf}}$", color="forestgreen", lw=self.linewidth)

        ax.plot(num_replications_list, pstars, "-", label=r"$p^*$", color="crimson", lw=self.linewidth )

        ax.legend(loc="lower left", fontsize=self.fontsize)
        ymin, ymax = ax.get_ylim()
        ax.set_ylim(ymin-1,ymax)
        ax.set_xlabel(r"\# replications $M$ (\# samples $N$ = %d fixed)" % num_samps_fixed, fontsize=self.fontsize)
        ax.set_xscale("log")
        for label in (ax.get_xticklabels() + ax.get_yticklabels()):
            label.set_fontsize(self.fontsize_axis)
        ...
        fig, ax = plt.subplots()
        ax.plot( num_replications_list, gaphats_plot, "-", label=r"$\hat{\Delta}_{M,N}$", color="blue", lw=self.linewidth )
        ax.plot( num_replications_list, gaps, "-", label=r"$\Delta$", color="crimson", lw=self.linewidth )
    ...
    def test_least_squares(self, m, n, want_plot_saa_quality=False):
        ...
        Aij_mu, Aij_cov = 1, 1
        bi_mu, bi_cov = -1, 1
        A = cvx.stochastic.RandomVariableFactory().create_normal_rv( Aij_mu, Aij_cov, (m,n) )
        b = cvx.stochastic.RandomVariableFactory().create_normal_rv( bi_mu, bi_cov, (m,1) )
        ...
        if want_plot_saa_quality:
            num_samples_list = [1,10,100,1000] # Cut (too costly): ,10000].  Not used for any other tests.
            num_replications = 10 # Not used for any other tests
            alpha = 0.05 # Not used for any other tests
            fn = "fig1.png"

            self.test_least_squares_plot_saa_quality(A, b, P, q, r, num_samples_list, fn, num_replications, alpha)
            print("")
        ...
        (fhatmeans, fhatvars, fstars) = self.test_least_squares_plot_cost_ests(A, b, P, q, r, x, num_replications_list, alpha, fn)
        print("")

        # Make lots of other plots
        num_replications_list = [10,100]
        num_samples_list = [10,100]
        fn1, fn2, fn3, fn4, fn5 = "fig3.png", "fig4.png", "fig5.png", "fig6.png", "fig12.png"

        self.test_least_squares_plot_optval_ests(A, b, P, q, r, num_replications_list, num_samples_list, alpha, fn1, fn2, fn3, fn4, fn5, fhatmeans, fstars) # Cut (no longer used): (pstarhatmeans, pstarhatvars, pstars) =
        print("")
    ...
    def test_QCQP(self, n=5, num_samples_list=[10,100], num_replications_list=[10,100], alpha=0.05, fn1="fig9.png", fn2="fig10.png", fn3="fig11.png"): # Cut (too costly): ,1000
        # Create problem data
        np.random.seed(4) # I had to play with the seed to get the QCQP constraint in the true problem below to be feasible (similar to http://web.cvxr.com/cvx/examples/cvxbook/Ch05_duality/html/qcqp.html)
        ...
        x = cvx.Variable(n)
        prob = cvx.Problem( cvx.Minimize(0.5*cvx.quad_form(x,P) + x.T*q0 + r0), [0.5*cvx.quad_form(x,P) + x.T*q1 + r1 <= 0] )
        prob.solve()
        pstar = prob.value

        # Compute the true cost
        xtilde = cvx.Variable(n)
        prob = cvx.Problem( cvx.Minimize(0), [0.5*cvx.quad_form(xtilde,P) + x.T*q1 + r1 <= 0] ) # Just get a feasible point
        prob.solve()
        ...
        for i in range(X): #range(X-1,-1,-1):
            ...
            (fhat, fhat_ucb, fhat_lcb) = self.test_QCQP_compute_fhat(q0, r0, xtilde, num_replications, alpha)
            fhats[i] = fhat
            ...
            for j in range(Y): #range(Y-1,-1,-1):
                ...
                # Estimate the true optimal value
                print("Minimizing Lagrangian [%d] times with [%d] Monte Carlo samples." % (num_replications, num_samples))
                (pstarhat, pstarhat_ucb, pstarhat_lcb) = self.test_QCQP_compute_pstarhat(q0, r0, q1, r1, x, num_replications, num_samples, alpha)
        ...
        print("Minimizing Lagrangian [%d] times with [%d] Monte Carlo samples." % (num_reps_fixed, num_samps_addl))
        (pstarhat_addl, pstarhat_addl_ucb, pstarhat_addl_lcb) = self.test_QCQP_compute_pstarhat(q0, r0, q1, r1, x, num_reps_fixed, num_samps_addl, alpha)
    ...
    def test_port_opt_plot_saa_quality(self, n=50, num_samples_list=[1,10,100,1000], fn_objvals="fig7.png", fn_probs="fig8.png", num_replications=10, alpha=0.05): # Cut (too costly): ,10000
        # Create problem data
        ...
        critval = UnconstrDiagnostics.get_critval(alpha, HowGetCritVal.T, num_replications-1)
        for num_samples in num_samples_list:
            print("Solving [%d] times a SAA to the stochastic port. opt. problem with [%d] Monte Carlo samples." % (num_replications, num_samples))
            pstarhats = []
            probs = []
            for i in range(num_replications):
                x = cvx.Variable(n)
                ...
                prob = cvx.Problem( cvx.Maximize(x.T*pbar), [cc <= beta, x.T*o == 1, x >= -0.1] )
        ...
        # Solve the true problem
        var = x.T*pbar >= scipy.stats.norm.ppf(1-beta) * cvx.norm2( scipy.linalg.sqrtm(Sigma)*x )
        ...
        cvar = x.T*pbar >= np.exp( -np.power(scipy.stats.norm.ppf(1-beta),2) / 2 ) / (np.sqrt(2*np.pi)*beta) * cvx.norm2( scipy.linalg.sqrtm(Sigma)*x )
        ...
        plt.rc("text", usetex=True)
        ax1.plot(num_samples_list, pstars_var, "-", label=r"$p^*$ (VaR)", color="forestgreen", lw=self.linewidth)

        ax1.plot(num_samples_list, pstars_cvar, "-", label=r"$p^*$ (CVaR)", color="crimson", lw=self.linewidth)

        ax1.plot(num_samples_list, pstarhat_avgs, "-", label=r"$\hat{p}^*_N$ (CVaR)", color="blue", lw=self.linewidth)
        ax1.fill_between( num_samples_list, pstarhat_ucbs, pstarhat_lcbs, facecolor="lightskyblue", alpha=0.5 )
        ...
        for label in (ax1.get_xticklabels() + ax1.get_yticklabels()): # See http://stackoverflow.com/questions/3899980/how-to-change-the-font-size-on-a-matplotlib-plot
            label.set_fontsize(self.fontsize_axis)
        ...
        plt.rc("text", usetex=True)
        ax2.plot(num_samples_list, prob_avgs, "-", label=r"$\mathop{\bf E{}} (-x^T \bar{p} / \alpha + 1)_+$", color="blue", lw=self.linewidth)
        ax2.fill_between( num_samples_list, prob_ucbs, prob_lcbs, facecolor="lightskyblue", alpha=0.5 )
        ax2.plot(num_samples_list, np.tile(beta,len(num_samples_list)), "-", label=r"$\beta$", color="crimson", lw=self.linewidth)
