...
class prob:
    ...
    def __init__(self, constr, num_samples, phi=Phi.HINGE):
        ...
        if self.phi == Phi.HINGE:
            self.alpha = Variable(1, nonneg=True)
    ...
    def __le__(self, beta):
        ...
        # in the "Note" above
        args = self.constr.args[0].args
        # for arg in :
        #     print(arg)
        # print(dir(self.constr.args[0]))
        constr_rearranged = args[1] - args[0]
        self.conservative = expectation(maximum(0.0, constr_rearranged + self.alpha), self.num_samples)
